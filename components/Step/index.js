...
const Badge = ({isAvailable, index}) => 
  aulas[index].available  ? <div className={`bg-${isAvailable[index] ? 'green' : 'gunmetal'} rounded-md pr-2 pl-2 pt-1 pb-1 text-${isAvailable[index] ? 'white' : 'green'} uppercase mt-2`}>
  Liberada
</div> : <></>
